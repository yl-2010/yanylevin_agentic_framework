...
```
education/<email>/
  meta.json                 # gradeFolder, onedriveRoot, optional hiddenFiles / visibleFiles for user-level dropped files
...
- **class.json:** `name`, `period` (A–H letter), `trimester`, optional `description` (markdown), optional `hiddenFiles` / `visibleFiles` (string arrays — dashboard file-tile visibility; `context.md` is hidden by default), optional extras
...
- **project.json:** `name`; optional `description` (markdown); optional `order` (number — Projects panel sorts ascending, then by name); optional `hiddenFiles` / `visibleFiles` (string arrays). No `period`, no `trimester`, not on the class schedule.
...
- **date.json:** `name`; `date`; optional `time`, `description` (markdown); optional `hiddenFiles` / `visibleFiles` (string arrays); optional `canvasLink` / `canvasId` (same Canvas button as todos, used for syllabus or calendar-event dates)
